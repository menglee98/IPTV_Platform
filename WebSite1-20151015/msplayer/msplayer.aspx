...
var max=parseInt('<%=(Session["max_user"])%>');
var count=parseInt('<%=(Session["now_user"])%>');
var video_server='<%=(sever_ip.ToString())%>';
var play_times=parseInt('<%=(Session["play_times"])%>');
var user_ip=('<%=(Session["User_IP_"])%>');
//var ip_city=('<%=(Session["ip_city"])%>');

var BroadStatus='<%=(Session["BroadStatus"])%>';
...
if(count>max){
	alert("已超過最大連線數");
	if(BroadStatus=="playing" || time_diff_val > 0){
		BroadBack();
	}
	else{
		GoBack();
...
var Video;
if(browser_type == "CT7010" || BroadStatus == "playing"){
...
function media_player(device,type){
    ...
    var wsx_ = '<param name="Filename" value="mms://' + video_server + '/media/' + file + '">';
    var http_ = '<param name="Filename" value="' + file + '">';
    var common_ = '<param name="Filename" value="mms://' + video_server + '/video/' + file + '">';    
    var common2_ = '<param name="Filename" value="mms://' + '120.96.153.81' + '/video/' + file + '">';//影片播放次數過多使用 
    var common3_ = '<param name="Filename" value="mms://' + '120.96.153.13' + '/video/' + file + '">';//依user_ip位置使用
    ...
    if(device == "CT7010" || BroadStatus == "playing"){
        if(type == "wsx"){
    ...
    if(BroadStatus != "playing" && time_diff_val > 0){
        media_obj += '<param name="CurrentPosition" value="' + time_diff_val + '">';
    }
